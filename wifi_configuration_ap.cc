...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    if (err == ESP_OK) {
        ...
        // Load YOUTUBE URL
        char youtube_url[256] = {0};
        size_t youtube_url_size = sizeof(youtube_url);
        err = nvs_get_str(nvs, "youtube_url", youtube_url, &youtube_url_size);
        if (err == ESP_OK) {
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t advanced_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            if (!this_->youtube_url_.empty()) {
                cJSON_AddStringToObject(json, "youtube_url", this_->youtube_url_.c_str());
            }
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Save YOUTUBE URL
            cJSON *youtube_url = cJSON_GetObjectItem(json, "youtube_url");
            if (cJSON_IsString(youtube_url) && youtube_url->valuestring) {
                this_->youtube_url_ = youtube_url->valuestring;
                err = nvs_set_str(nvs, "youtube_url", this_->youtube_url_.c_str());
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to save YOUTUBE URL: %d", err);
                }
            }
            ...
            ESP_LOGI(TAG, "Save youtube_url=%s", this_->youtube_url_.c_str());
